...
def is_safe_path(root_path: str, path: str) -> bool:
    ...
    target_real = os.path.realpath(os.path.join(root_real, path))
    ...
    async def handle_list_resources() -> list[types.Resource]:
        ...
        for root, dirs, files in os.walk(root_path, followlinks=False):
            dirs[:] = [
                ...
                and not is_ignored(root_path, os.path.join(root, d), ignore_patterns)
            ]
    ...
    async def handle_get_prompt(
        ...
        if os.path.getsize(full_path) > MAX_FILE_SIZE:
            raise McpError(
                types.ErrorData(
                    code=types.INVALID_PARAMS,
                    message="File too large to analyze",
                )
            )
    ...
    async def handle_call_tool(
        ...
        if name == "search-files":
            ...
            if args.file_pattern != "*":
                file_spec = pathspec.PathSpec.from_lines(
                    "gitwildmatch", [args.file_pattern]
                )
            ...
            for root, dirs, files in os.walk(root_path, followlinks=False):
                for file in files:
                    ...
                    if file_spec:
                        rel_path = os.path.relpath(full_path, root_path)
                        if not file_spec.match_file(rel_path):
                            continue
